...
We will use Jest and Supertest to run all tests against the real SQLite database. Sequelize models are never mocked.

- Tests load the real SQLite database via the path specified in `.env.test`
- `beforeEach` seeds test data via POST where write operations are tested; `afterEach` cleans up via DELETE
- Supertest drives requests through the full Express app stack (middleware, routing, controller, service, database)
- No mocking of Sequelize methods, models, or the database module
