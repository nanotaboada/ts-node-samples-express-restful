...
- The Dockerfile uses two stages: `builder` (installs all deps, compiles TypeScript, generates Swagger docs, prunes dev dependencies) and `runtime` (copies `dist/`, production `node_modules/`, `swagger.json`, `README.md`, `scripts/`, and `storage/` into `hold/`)
